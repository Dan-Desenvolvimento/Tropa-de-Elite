          ...
          <div className="rounded-2xl border border-dashed border-white/10 p-12 text-center">
            ...
            <p className="mt-4 text-sm text-zinc-500">
              {canCreateEvents
                ...
                : "Nenhum evento foi liberado para o seu acesso."}
            </p>
          </div>
        ) : (
            ...
            {events.map((event) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="rounded-full border border-white/8 bg-white/5 px-3 py-1 text-xs text-zinc-400">
                    {event.event_status}
                  </span>
                </div>
                ...
                <div className="mt-6 flex flex-wrap gap-2 border-t border-white/8 pt-5">
                  {event.can_edit_event ? (
                    <Action
                      href={`/admin/eventos/${event.event_id}`}
                      icon={Settings2}
                    >
                      Gerenciar
                    </Action>
                  ...
                  {event.can_checkin ? (
                    <Action
                      href={`/admin/eventos/${event.event_id}/checkin`}
                      icon={ScanLine}
                    >
                      Check-in
                    </Action>
                  ...
                  event.can_anonymize_registrations ? (
                    <Action
                      href={`/admin/eventos/${event.event_id}/inscritos`}
                      icon={UsersRound}
                    >
                      Inscritos
                    </Action>
                  ...
                  {event.can_view_reports ? (
                    <Action
                      href={`/admin/eventos/${event.event_id}/relatorios`}
                      icon={BarChart3}
                    >
                      Relatórios
                    </Action>
                  ...
                  {event.can_view_logs ? (
                    <Action
                      href={`/admin/eventos/${event.event_id}/logs`}
                      icon={History}
                    >
                      Histórico
                    </Action>
...
function Action({
  ...
  return (
    <Link
      href={href}
      className="inline-flex items-center gap-2 rounded-lg border border-white/8 bg-white/5 px-3 py-2 text-xs font-medium text-zinc-300 hover:bg-white/10"
    >
