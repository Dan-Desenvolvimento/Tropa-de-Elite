...
export default async function RegistrationDetailPage({ params }: PageProps<"/admin/eventos/[id]/inscritos/[registrationId]">) {
  ...
  return <main><PageHeader eyebrow={event.name} title={registration.full_name} description={registration.ticket_code} actions={<RegistrationActions eventId={id} registrationId={registration.id} checkedIn={Boolean(registration.checked_in_at)} cancelled={registration.status === "cancelled"} />} /><div className="grid gap-6 p-5 sm:p-8 lg:grid-cols-2 lg:p-10"><Section title="Dados da inscrição"><Row label="E-mail" value={registration.email} /><Row label="Telefone" value={registration.phone} /><Row label="Cidade" value={registration.city} /><Row label="Status" value={registration.status} /><Row label="Inscrição" value={new Date(registration.registered_at).toLocaleString("pt-BR")} /><Row label="Check-in" value={registration.checked_in_at ? new Date(registration.checked_in_at).toLocaleString("pt-BR") : "Pendente"} /></Section><Section title="Consentimentos"><Row label="Política" value={`Aceita em ${new Date(registration.privacy_consent_at).toLocaleString("pt-BR")}`} /><Row label="Versão" value={registration.privacy_policy_version} /><Row label="Comunicações" value={registration.communications_consent ? "Aceito" : "Não aceito"} />{registration.cancelled_at ? <Row label="Cancelamento" value={`${new Date(registration.cancelled_at).toLocaleString("pt-BR")} · ${registration.cancellation_reason ?? "Sem motivo"}`} /> : null}</Section><Section title="Respostas personalizadas"><pre className="overflow-auto whitespace-pre-wrap text-xs leading-6 text-zinc-400">{JSON.stringify(registration.custom_answers ?? {}, null, 2)}</pre></Section><Section title="Histórico de e-mails">{((emails ?? []) as EmailLog[]).length === 0 ? <Empty /> : ((emails ?? []) as EmailLog[]).map((email) => <div key={email.id} className="border-b border-white/8 py-3 last:border-0"><div className="flex justify-between gap-3"><span className={email.status === "sent" ? "text-emerald-400" : email.status === "failed" ? "text-red-400" : "text-amber-400"}>{email.status}</span><span className="text-xs text-zinc-600">{new Date(email.sent_at ?? email.created_at).toLocaleString("pt-BR")}</span></div>{email.error_message ? <p className="mt-2 text-xs text-zinc-600">{email.error_message}</p> : null}</div>)}</Section><Section title="Histórico de check-in">{((checkins ?? []) as CheckinLog[]).length === 0 ? <Empty /> : ((checkins ?? []) as CheckinLog[]).map((log) => <div key={log.id} className="flex justify-between gap-3 border-b border-white/8 py-3 text-sm last:border-0"><span className="text-zinc-300">{log.result} · {log.method}</span><span className="text-xs text-zinc-600">{new Date(log.created_at).toLocaleString("pt-BR")}</span></div>)}</Section></div></main>;
}
