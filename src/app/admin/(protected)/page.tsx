...
export default async function AdminDashboardPage() {
  ...
  const canCreateEvents =
    staff.isOwner || staff.canCreateEvents;

  const activeEvents = events.filter((event) =>
    ["open", "closed", "sold_out"].includes(
      event.event_status,
    ),
  );
  const futureEvents = events.filter(
    (event) => event.is_future,
  );
  ...
  const totalRegistrations = events.reduce(
    (sum, event) =>
      sum + Number(event.confirmed_count),
    0,
  );
  const totalCheckins = events.reduce(
    (sum, event) => sum + Number(event.checkin_count),
    0,
  );
  ...
  return (
    <main>
      <PageHeader
        ...
        title="Visão geral"
        description="Acompanhe inscrições e entradas conforme as suas permissões."
        actions={
          canCreateEvents ? (
            <Link
        ...
        <section>
          ...
          {events.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-sm text-zinc-500">
              Nenhum evento disponível para o seu acesso.
            </div>
          ) : (
            <div className="grid gap-4 xl:grid-cols-2">
              {events.slice(0, 6).map((event) => (
                <Link
                  key={event.event_id}
                  href={getEventDestination(event)}
                ...
                >
                  ...
                  <div className="mt-5 grid grid-cols-3 gap-3 text-sm">
                    ...
                    <SmallMetric
                      label="Aguardando"
                      value={
                        Number(event.confirmed_count) -
                        Number(event.checkin_count)
                      }
                    />
                  </div>
                </Link>
...
function getEventDestination(event: EventSummary) {
  if (event.can_edit_event) {
    return `/admin/eventos/${event.event_id}`;
  }
  if (event.can_checkin) {
    return `/admin/eventos/${event.event_id}/checkin`;
  }
  if (
    ...
    return `/admin/eventos/${event.event_id}/inscritos`;
  }
  if (event.can_view_reports) {
    return `/admin/eventos/${event.event_id}/relatorios`;
  }
  return `/admin/eventos/${event.event_id}/logs`;
}
