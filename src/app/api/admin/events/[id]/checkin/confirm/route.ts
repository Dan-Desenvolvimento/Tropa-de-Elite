...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ success: false, code: "INVALID_TOKEN" }, { status: 400 });
  const value = parsed.data.value.startsWith("EVENT:") ? parsed.data.value.slice(6) : parsed.data.value;
  if (parsed.data.forceWaitlist && !(await hasEventRole(id, ["admin"]))) {
    return NextResponse.json({ success: false, code: "UNAUTHORIZED" }, { status: 403 });
  }
  const ipHash = hashRequestIdentifier(getRequestIp(request));

  const admin = createAdminClient();
  const { data: rateLimit } = await admin.rpc("consume_rate_limit", {
    rate_scope: "operator_checkin",
    rate_key_hash: `${staff.id}:${ipHash}`,
    rate_max_attempts: 120,
    rate_window_seconds: 60,
  });
  if (!(rateLimit as { allowed?: boolean } | null)?.allowed) {
    return NextResponse.json({ success: false, code: "RATE_LIMITED" }, { status: 429 });
  }

  const supabase = await createClient();
  const { data, error } = await supabase.rpc("process_event_checkin", {
    target_event_id: id,
    ticket_value: value,
  ...
  if (error) return NextResponse.json({ success: false, code: "CHECKIN_FAILED" }, { status: 500 });
  const result = data as { success?: boolean; code?: string; registration_id?: string } | null;
  if (parsed.data.method === "manual" && result?.code === "CHECKIN_SUCCESS" && result.registration_id) {
