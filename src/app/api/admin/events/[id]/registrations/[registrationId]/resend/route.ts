...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string; registrationId: string }> }) {
  ...
  const { count } = await supabase.from("email_logs").select("id", { count: "exact", head: true }).eq("registration_id", registrationId).gte("created_at", oneMinuteAgo);
  if ((count ?? 0) > 0) return NextResponse.json({ success: false, message: "Aguarde um minuto antes de reenviar." }, { status: 429 });

  await supabase.from("audit_logs").insert({ actor_id: staff.id, event_id: id, action: "ticket_resent", entity_type: "registration", entity_id: registrationId, metadata: {} });
  after(async () => { await sendTicketConfirmation(registrationId); });
