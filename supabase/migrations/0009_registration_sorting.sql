drop function if exists public.list_event_registrations_admin(
  uuid, text, public.registration_status, integer, integer
);
...
begin
  ...
  order by
    ...
    case when sort_order not in ('oldest', 'name_asc', 'name_desc') then f.registered_at end desc,
    f.id asc
...
revoke all on function public.list_event_registrations_admin(
  uuid, text, public.registration_status, integer, integer, text
) from public, anon;
